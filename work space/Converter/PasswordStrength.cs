...
namespace work_space
{
    ...
    internal class PasswordCheck
    {
        ...
        {
            int score = 0;
            if (String.IsNullOrEmpty(password) || String.IsNullOrWhiteSpace(password)) return 0;
            if (!HasMinimumLength(password, 4)) return 0;
            if (HasMinimumLength(password, 4)) score++;
